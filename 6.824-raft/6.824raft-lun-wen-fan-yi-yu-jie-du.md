...
![img](https://files.gitbook.com/v0/b/gitbook-x-prod.appspot.com/o/spaces%2FVcxvX8LyXwh6YqgzHB43%2Fuploads%2FePBStgOCOWhRjezUFXew%2Fimage.png?alt=media\&token=4e81c2fd-ac13-4d8c-a0d0-cf3d2dc92443)
...
上一节我们讲到了Raft如何选举领导者以及复制日志。然而仅仅只有这两个机制无法保证每个状态机能够按序执行相同的命令。

例如，领导者提交一些日志条目的时候，跟随者可能处于一个不可用的状态，它可能被选举为领导者，并且用心点日志条目覆盖。

这一节我们添加了一个限制，这个限制限制了哪个server能够成为领导者。这个限制保证任意任期中的领导者包含所有之前任期提交过的日志。

**5.4.1 选举限制**

候选者在发起选举的时候，会和集群中半数以上的服务器进行通信。如果发现候选者的日志早于于其他服务器的日志，那么候选者将会保持保持所有的commit
